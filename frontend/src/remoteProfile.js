...
export const getRemoteProfile = async (remoteUrl, accessToken, authEnabled) => {
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        `frontend remoteProfile.js getRemoteProfile response json: ${JSON.stringify(
          apiResponse?.profile
        )}`
      );
      ...
      console.log(
        `frontend remoteProfile.js getRemoteProfile api Fetch error text: ${response?.statusCode} ${textError}`
      );
    ...
    console.log(
      `frontend remoteProfile.js getRemoteProfile caught error = ${error?.message}`
    );
    return {
      error: {
